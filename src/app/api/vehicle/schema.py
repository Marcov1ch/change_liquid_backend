...
class UpdateVehicleData(BaseModel):
    ...
    transmission_interval_km: int | None = Field(None, ge=10000, description="Интервал замены масла АКПП (км)")
    brake_interval_km: int | None = Field(None, ge=10000, description="Интервал замены тормозной жидкости (км)")
    coolant_interval_km: int | None = Field(None, ge=10000, description="Интервал замены антифриза (км)")
    power_steering_interval_km: int | None = Field(None, ge=10000, description="Интервал замены жидкости ГУРа (км)")
    differential_oil_interval_km: int | None = Field(None, ge=10000, description="Интервал замены масла в редукторе (км)")
    ...
    def validate_plate_number(cls, v: str | None) -> str | None:
        ...
        if not any(re.match(p, cleaned) for p in patterns):
            raise ValueError('Некорректный формат госномера. Допустимые форматы: А123АА178 (РФ) или 1234AB7 (РБ)')
        return cleaned
